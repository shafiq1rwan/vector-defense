...
- **Hero line-up** — a pre-match squad screen (all modes) where you pick up to 6 of your unlocked heroes to deploy; only the chosen squad docks during the match, so the mobile dock stays uncluttered and every pick is a real decision. Structures (REACTOR, BASTION) always deploy and don't count against the cap. The screen is skipped automatically while you have fewer heroes than the cap
