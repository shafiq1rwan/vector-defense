...
- **Quick Play** — a single **CASUAL** run (no difficulty selector): mission-prep screen with road-topology previews of all 4 maps (spawn/exit tags like "2 SPAWNS · 1 EXIT"); endless mode after wave 20. The challenge comes from the limits, not raw HP — a **hero line-up** caps your squad and a **deployment cap** limits how many towers you can field (starts at 8, grows as you clear waves up to 16) so every placement is a choice, shown as a TOWERS x/cap chip in the HUD
- **Daily Challenge** — a short date-seeded run (same map, modifier and **special rule** for everyone that day, CASUAL, 5 waves) reachable from the calendar button on the title. Each day imposes its own deployment constraint — **SOLO** (one hero only), **STRIKE TEAM** (a fixed squad with set counts, e.g. SLY ×2 · BRUNO ×1), **LIMITED POOL** (field any few from a small roster), or a **HANDICAP** (no abilities, a shoestring budget, or only 5 lives) — so it plays nothing like Quick Play. The screen shows the day's condition, battlefield and a countdown to the next reset; clear it once a day for bonus cores + XP and chase your best wave
...
- **Hero line-up** — a pre-match squad screen (Quick Play) where you pick up to 6 of your unlocked heroes to deploy; only the chosen squad docks during the match, so the mobile dock stays uncluttered and every pick is a real decision. The REACTOR is now one of the picks. The BASTION always deploys but only **one** can stand at a time, so it can't be spammed. The screen is skipped automatically while you have fewer heroes than the cap
